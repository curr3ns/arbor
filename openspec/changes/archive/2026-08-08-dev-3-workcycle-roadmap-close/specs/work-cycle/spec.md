...
Implementing this change SHALL modify only `.claude/skills/arbor-auto-work/SKILL.md` and the OpenSpec artifacts for this change. The skills `arbor-opsx-auto`, `arbor-auto-developer`, `arbor-auto-roadmap`, and `arbor-project-scaffold` SHALL NOT be created, modified, moved, or deleted, and no new skill SHALL be added. No `docs/roadmaps/archive/` directory, placeholder, or `.gitkeep` SHALL be committed by this change. `docs/roadmaps/roadmap-native-workcycles.md` SHALL be read for context and SHALL NOT be modified.

#### Scenario: No other path is touched

- **WHEN** `git status --porcelain` is inspected after implementation
- **THEN** the only path listed outside `openspec/` is a modification to `.claude/skills/arbor-auto-work/SKILL.md`
- **AND** no path under `.claude/skills/arbor-opsx-auto/`, `arbor-auto-developer/`, `arbor-auto-roadmap/`, or `arbor-project-scaffold/` appears

#### Scenario: The archive directory is not materialized here

- **WHEN** the working tree is inspected after implementation
- **THEN** `docs/roadmaps/archive/` has not been created or committed, because the skill creates it on demand and project scaffolding owns creating it for new repos
